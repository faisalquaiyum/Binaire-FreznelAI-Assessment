...
export class ModelCatalog {
  ...
  select(query: CatalogQuery): ModelRecord[] {
    ...
    const filtered = this.models.filter((model) => {
      ...
      const fileMatch = model.safetensorFiles >= query.filters.safetensorMin && model.safetensorFiles <= query.filters.safetensorMax;
    ...
    return filtered.sort((left, right) => {
      ...
      if (query.sort === 'files-desc') return right.safetensorFiles - left.safetensorFiles;
